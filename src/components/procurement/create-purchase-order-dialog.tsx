...
export function CreatePurchaseOrderDialog({ open, onOpenChange, onSuccess }: CreatePurchaseOrderDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      form.setValue("poNumber", `PO-${Math.floor(100000 + Math.random() * 900000)}`);
    }
  }, [open, form]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] flex flex-col">
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 overflow-y-auto pr-2 pb-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-muted/20 p-4 rounded-lg border border-dashed border-border/60">
            <div className="space-y-2">
              <Label htmlFor="vendorId" className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Vendor</Label>
              <select
                id="vendorId"
                {...form.register("vendorId")}
                className="w-full flex h-10 rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background transition-all focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none disabled:opacity-50"
              >
            ...
            <div className="space-y-2">
              <Label htmlFor="poNumber" className="text-xs font-bold uppercase tracking-wider text-muted-foreground">PO Number</Label>
              <Input 
                id="poNumber" 
                {...form.register("poNumber")} 
                readOnly 
                className="bg-muted/50 font-mono font-bold text-primary cursor-not-allowed border-muted"
              />
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              {fields.map((field, index) => (
                <div key={field.id} className="flex flex-col sm:flex-row gap-4 items-end p-4 rounded-xl bg-card border border-border shadow-sm transition-all hover:shadow-md animate-in fade-in slide-in-from-right-4 duration-300">
                  <div className="flex-1 w-full space-y-2">
                    ...
                    <select
                      ...
                      className="w-full flex h-10 rounded-lg border border-input bg-background px-3 py-2 text-sm transition-all focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none"
                      onChange={(e) => {
