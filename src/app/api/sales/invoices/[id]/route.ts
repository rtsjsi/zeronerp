...
export const GET = withAuth(async (_req, ctx) => {
  try {
    const { id } = ctx.params;
    const invoice = await SalesService.getSalesInvoiceById(ctx.storeId, id);
    ...
    console.error("[Sales Invoice GET API Error]", err);
...
export const PATCH = withAuth(async (req, ctx) => {
  try {
    ...
    return apiSuccess(invoice, "Sales Invoice updated and stock adjusted");
  } catch (err) {
    console.error("[Sales Invoice PATCH API Error]", err);
